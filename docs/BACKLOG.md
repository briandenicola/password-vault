...
| OFF-5 | P2 | M | 🚧 **In progress (parallel stack scaffolded; cutover pending).** **Cutover runbook + parallel stack.** ✅ `infrastructure-next/` — an isolated, additive Terraform root for the vault-next stack (separate Cosmos account + `Passwords`/`VaultKeys`, own remote-state key, random-suffixed names, E2EE-native defaults, `WEBAUTHN_RP_ID=denicolafamily.com`, custom domain `vault.denicolafamily.com` gated behind DNS); production `infrastructure/` untouched. **Remaining:** operator applies it, runs `MIG-4` import, tests E2EE (`OFF-4` 2b/2c) on it, then cuts over — brief write-freeze → fresh `backup` + full re-import → `MIG-5` parity → flip the SWA/DNS URL → keep old URL live as rollback. **RP ID is locked to the registrable parent (`denicolafamily.com`)** so passkeys survive the switch. Design: [`design/blue-green-rebuild.md`](design/blue-green-rebuild.md) §5, §8. |
