...
| MIG-4 | P2 | M–L | 📋 **Planned (blue-green rebuild).** **Import / transform into a new E2EE-native store.** One-shot tool (extend `vault-migrate` with `import`, or a sibling): read a `backup` JSON snapshot → **decrypt** each secret + history with the existing `Encryptor` (no second crypto impl) → **re-encrypt under the vault DEK** (`OFF-4` envelope) → write the new schema (DEK-encrypted blobs + the §5B vault-key-record; no server-readable secret, no v1 `HmacHash`). Plaintext stays in memory only. Needs the new-store schema locked first. Design: [`design/blue-green-rebuild.md`](design/blue-green-rebuild.md) §3–4. |
| MIG-5 | P2 | S–M | 📋 **Planned (blue-green rebuild).** **Cutover parity verification.** Before flipping the URL, prove every secret round-trips: account + per-account history **count parity** old→new, and **value parity** (`decrypt_new == decrypt_old`) for every entry; any mismatch/undecryptable blocks cutover (reuse the `verify` decrypt-check across stores). Spot-check known `CR-1` non-ASCII by hand. *(Honors "no hopeful patches" — proves the exact migration path.)* Design: [`design/blue-green-rebuild.md`](design/blue-green-rebuild.md) §6. |
...
| OFF-5 | P2 | M | 📋 **Planned (blue-green rebuild).** **Cutover runbook + parallel stack.** Stand up a separate stack with its own datastore (parallel Terraform workspace/state), import via `MIG-4`, test E2EE end-to-end on it (`OFF-4` 2b/2c with the flag on there only), then cut over: brief write-freeze → fresh `backup` + full re-import → `MIG-5` parity → flip the SWA/DNS URL → keep old URL live as instant rollback. **Decide the WebAuthn RP ID = final production registrable domain up front** so passkeys survive the URL switch (else plan a one-time post-cutover re-enroll = a DEK re-wrap). Design: [`design/blue-green-rebuild.md`](design/blue-green-rebuild.md) §5, §8. |
